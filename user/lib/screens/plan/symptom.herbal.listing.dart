...
class SymptomHerbalListing extends StatefulWidget {
  final Symptoms symptom;

  const SymptomHerbalListing({Key? key, required this.symptom}) : super(key: key);

  @override
  _SymptomHerbalListingState createState() => _SymptomHerbalListingState();
}

class _SymptomHerbalListingState extends State<SymptomHerbalListing> {
  Store? _store;
  Box<Plan>? _planBox;
          ...
          builder: (_, snapshot) {
            ...
            if (_selectedPlanName == null) {
              for (final plan in plans) {
                if (plan.symptomOrCondition.toLowerCase() == widget.symptom.name.toLowerCase()) {
